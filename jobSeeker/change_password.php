...
if(isset($_GET['op']) && isset($_GET['np']) && isset($_GET['cp'])) {
    ...
    if ($count == 0) {
        ...
        echo "<script>window.location.href='http://localhost/worklink/jobSeeker/change_password.php';</script>";
    } else {
        if ($new_password == $confirm_password) {
            $update_query = "UPDATE users SET password = '$new_password' WHERE user_name = '$un'";
            if (mysqli_query($con, $update_query)) {
                echo "<script>alert('Password updated successfully');</script>";
                echo "<script>window.location.href='http://localhost/worklink/jobSeeker/Manage_profile.php';</script>";
